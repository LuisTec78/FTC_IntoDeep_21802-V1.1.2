...
public class MecanumJkvel extends LinearOpMode {
    @Override
    public void runOpMode() {
        ...
        float posy = gamepad1.left_stick_y * -1;
        float posx = gamepad1.left_stick_x * -1;
        ...
        while (opModeIsActive()){ //corre mientras el programa este activo
            if (gamepad1.left_stick_y < 0) { //corre si presionas el boton a (X en el control de play)
                //asigna las potencias a los motores para avanzar
                motorLu.setPower(posy);
                motorRu.setPower(posy);
                motorLd.setPower(posy);
                motorRd.setPower(posy);
